...
from data_loaders.humanml.scripts.motion_process import recover_from_ric, recover_rot
...
def main():
    ...
    for rep_i in range(1):
        ...
        j3d = sample[0].permute(2, 0, 1)
        j = torch.zeros(196, 24, 3)
        j[:, :13] = j3d[:, :13]
        j[:, 15:] = j3d[:, 13:]
        j[:, 15] = j3d[:, 15]
        j[:, 16] = j3d[:, 13]
        ...
        j[:, 1:] = j[:, 1:] - j[:, :1]
        j[:, 0] = 0
        body_model_cpu.view_joint([j])
        js.append(j)
        
        # inpainted_motion = sample[1].cpu()
        # j = torch.cat((sample[2].unsqueeze(1).cpu(), sample[1].cpu()), dim=1)
        # body_model_cpu.view_motion([j])
        # js.append(j)
    ...
    for rep_i in range(2):
        ...
        if rep_i == 1:
            ...
            model_kwargs['y']['inpainting_mask'] = torch.tensor(humanml_utils.SMPL_IMU_BODY_MASK, dtype=torch.bool,
                                                                device=inpainted_motion.device)  # True is lower body data
            # model_kwargs['y']['inpainting_mask'] = torch.tensor(humanml_utils.HML_IMU_BODY_MASK, dtype=torch.bool,
            #                                                     device=inpainted_motion.device)  # True is lower body data
            model_kwargs['y']['inpainting_mask'] = model_kwargs['y']['inpainting_mask'].unsqueeze(0).unsqueeze(
                -1).unsqueeze(-1).repeat(inpainted_motion.shape[0], 1, inpainted_motion.shape[2], inpainted_motion.shape[3])
    ...
    # body_model_cpu.view_joint(js)
    body_model_cpu.view_motion(js)
